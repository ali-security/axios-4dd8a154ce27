...
export default isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    // options.maxBodyLength itself. Every other transport (native http/https,
    // a user-supplied custom transport) needs the explicit byte-counting
    // pipeline below to enforce maxBodyLength on streamed uploads.
      ...
      if (responseType === 'stream') {
        ...
        responseStream.on('data', function handleStreamData(chunk) {
          ...
          if (maxContentLength > -1 && totalResponseBytes > maxContentLength) {
            ...
            reject(new AxiosError('maxContentLength size of ' + maxContentLength + ' exceeded',
    ...
    if (utils.isStream(data)) {
      ...
      // does not apply options.maxBodyLength itself (native http/https and
      // user-supplied custom transports). The follow-redirects transport
      // enforces it on the redirected HTTP/1 path. See GHSA-5c9x-8gcm-mpgx.
